...
<script type="text/javascript">
    ...
    $(function () {
        //上级资源下拉列表
        $("#pId").combotree({
            url: '<c:url value="${ctx}/sys/resource/treeList?resourceType=parentList"/>',
            idField: 'resourceNo',
            textField: 'name',
            lines: true,
...
//                $("#pId").combotree('tree').tree('expandAll');
                var resNo = $("[name='resNo']").val();
                if(resNo != null && resNo != "" && resNo != undefined) {
                    var extendNode = $('#pId').combotree('tree').tree('find', resNo);
                    $("#pId").combotree('tree').tree('expandTo',extendNode.target).tree("select",extendNode.target);
                }
            }
        });
...
<input type="hidden" name="resNo" value="${resource.resourceNo}"/>